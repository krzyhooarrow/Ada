...
procedure Main is
   ...
   begin
      loop
         ...
         inform("I have done the task " & Float'Image(newProduct.value));
   ...
   begin
      loop
         ...
         inform("Client bought a product  " & Float'Image(newProduct.value));
...
begin
   if (Ada.Command_Line.Argument_Count < 1) then
      Put_Line("Put -t to talkative or -c / default to calm");
   else
   ...
   if (modee = CALM) then
 Put_Line("put t to see task , or s to see storage");
      loop
         declare
            S : String(1..40);
            Last : Natural;
